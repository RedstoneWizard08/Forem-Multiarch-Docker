...
printf "${BLACK}${BG_CYAN} INFO ${RESET} ${BLUE}Changing permissions on PostgreSQL, Redis, and ElasticSearch data directories...${RESET}\n"

chown -R postgres:postgres /var/lib/postgresql/13/main
chmod -R a+rwx /var/lib/postgresql/13/main
chown -R root:root /var/lib/redis
chown -R root:root /var/lib/elasticsearch

printf "${BLACK}${BG_CYAN} INFO ${RESET} ${BLUE}Initializing PostgreSQL...${RESET}\n"

chmod -R a+rwx /var/lib/postgresql/13/main
sudo -u postgres /usr/lib/postgresql/13/bin/initdb -D /var/lib/postgresql/13/main

printf "${BLACK}${BG_CYAN} INFO ${RESET} ${BLUE}Initializing ElasticSearch...${RESET}\n"

cp -r /elastic-temp/* /var/lib/elasticsearch
...
sudo -u postgres createuser -s ${S_POSTGRES_USER}
sudo -u postgres psql -c "ALTER USER ${S_POSTGRES_USER} WITH PASSWORD '${S_POSTGRES_PASSWORD}';"
sudo -u postgres psql -c "CREATE DATABASE ${S_POSTGRES_USER};"
